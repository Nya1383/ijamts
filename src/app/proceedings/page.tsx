...
export default function Proceedings() {
  ...
  const fetchArticles = async () => {
    try {
      ...
      // Fetch articles from proceedings
      const articlesRef = collection(db, "articles");
      const queryProceedings = query(
        articlesRef,
        orderBy("approvedDate", "desc")
      );
      const snapshot = await getDocs(queryProceedings);

      const proceedingsArticles: Article[] = [];
      
      snapshot.forEach((doc) => {
        const data = doc.data();
        if (data.status === "approved" && data.issue === "proceedings") {
          proceedingsArticles.push({
            id: doc.id,
            name: data.name,
      ...
      setArticles(proceedingsArticles);
      
    } catch (error) {
      console.error("Error fetching articles:", error);
      toast.error("Failed to load conference proceedings");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="mt-2 text-[var(--secondary-text)]">
            Latest conference proceedings and research papers
          </p>
        </div>
                    ...
                    <tr key={article.id}>
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        ...
                        <div className="text-xs text-[var(--secondary-text)]">
                          {article.conferenceDate && new Date(article.conferenceDate).toLocaleDateString()}
                          {article.conferenceLocation && ` • ${article.conferenceLocation}`}
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[var(--background)] p-6 rounded-lg w-full max-w-2xl mx-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-sm font-medium text-[var(--secondary-text)] mb-1">Conference Details</h3>
                <p className="text-[var(--foreground)]">
                  {selectedArticle.conferenceName}
                  {selectedArticle.conferenceDate && ` • ${new Date(selectedArticle.conferenceDate).toLocaleDateString()}`}
                  {selectedArticle.conferenceLocation && ` • ${selectedArticle.conferenceLocation}`}
                </p>
              </div>
